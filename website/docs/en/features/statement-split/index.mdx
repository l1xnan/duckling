...
- With a **non-empty selection**, the selection is executed (toggle ignored), except when the selection is a single CTE identifier — then Duckling previews that CTE (`SELECT * FROM <cte>`). See [SQL editor](/features/sql-editor#preview-a-cte).

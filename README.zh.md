...
- **数据浏览** — 基于 Canvas 渲染的结果网格，支持分页、隐藏列、虚拟计算列、转置、结果过滤与单元格值查看。
- **SQL 编辑器** — 基于 Monaco，具备表/列感知的自动补全，以及运行 / 格式化 / `EXPLAIN` 与 SQL 书签功能；可选**按语句分割**（高亮并仅执行光标所在语句）。详见 [当前语句执行](#当前语句执行)。
- **SQL 模板变量** — 在 SQL 中写入 Jinja2 风格 `{{ variable }}` 占位符，通过 `/* @vars */` YAML 块注释声明值，一次运行多值查询（笛卡尔积 → 多个结果标签页）。详见 [SQL 模板](#sql-模板)。
- **透视表** — 通过行维 / 列维与度量（`count` / `sum` / `avg` / `min` / `max`）构建透视，提供高基数警告、复制 SQL、前端行/列百分比展示与列头排序。
- **列画像** — 每列的统计数据：总数、空值比例、去重数、最大/最小值以及高频取值。
- **按列统计** — 单列的值分布（计数 + 占全部匹配行的占比），以表格加柱状图呈现。
...
- **设置套件** — 外观、编辑器、SSH 配置、快捷键、SQL 格式化、导入导出选项以及应用内更新。
...
- 有**非空选区**时执行选区内容（与开关无关）；若只选中单个 CTE 名，则预览该中间表。
...
- `Mod+B` — 切换侧边栏
- `Mod+/` — 快捷键帮助
- `Mod+Enter` — 运行 SQL（分割关闭：全文；分割开启：当前语句；光标在 CTE 名上：预览该中间表）

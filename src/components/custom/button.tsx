...
export const TooltipButton = React.forwardRef<HTMLButtonElement, IconButtonProps>(
  ({ tooltip, icon, children, className, tooltipProps, active, ...props }, ref) => {
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          ></TooltipTrigger>
          {tooltip ? <TooltipContent {...(tooltipProps ?? {})}>{tooltip}</TooltipContent> : null}
        </Tooltip>
      </TooltipProvider>
    );
  },

...
export class HighlightHeaderWhenSelectCellPlugin
  implements pluginsDefinition.IVTablePlugin
{
  ...
  updateHighlight() {
    ...
    for (let i = 0; i < selectRanges.length; i++) {
      ...
      let colHeaderRange: CellRange | undefined = undefined;
      let rowHeaderRange: CellRange | undefined = undefined;
      ...
      if (
        this.pluginOptions?.colHighlight !== false &&
        colHeaderRange &&
        !this.colHeaderRanges.find((range) =>
          isSameRange(range, colHeaderRange),
        )
      ) {
      ...
      if (
        this.pluginOptions?.rowHighlight !== false &&
        rowHeaderRange &&
        !this.rowHeaderRanges.find((range) =>
          isSameRange(range, rowHeaderRange),
        )
      ) {
        this.table.arrangeCustomCellStyle(
